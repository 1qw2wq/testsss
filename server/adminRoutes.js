...
function registerAdmin(app, ctx) {
  ...
  app.post('/api/admin/bulk', requireAdmin, (req, res) => {
    ...
    for (const id of ids) {
      ...
      const result = store.updateRow(collection, id, { status }, { type: TYPE_OF[collection], ...log });
  ...
  app.post('/api/admin/applications', requireAdmin, (req, res) => {
    ...
    const info = stmts.insertApplication.run({ name, wc, org, interests, msg });
    if (status !== 'new' || notes) store.updateRow('applications', info.lastInsertRowid, { status, notes }, { type: 'application', action: status !== 'new' ? 'status' : 'updated', summary: status !== 'new' ? `${name} marked ${status}` : `Note added on ${name}` });
  ...
  app.patch('/api/admin/applications/:id', requireAdmin, (req, res) => {
    ...
    const result = store.updateRow('applications', previous.id, patch, { type: 'application', ...activityFor('applications', { ...previous, ...patch }, previous, patch) });
    res.json({ ok: true, item: result });
  });

  app.post('/api/admin/pledges', requireAdmin, (req, res) => {
    ...
    if (status !== 'pledged' || notes) store.updateRow('pledges', info.lastInsertRowid, { status, notes }, { type: 'pledge', action: 'status', summary: status === 'received' ? `${name}'s ${qty} books marked received` : `${name}'s pledge marked ${status}` });
    res.status(201).json({ ok: true, item: rowFor('pledges', info.lastInsertRowid) });
  });

  app.patch('/api/admin/pledges/:id', requireAdmin, (req, res) => {
    ...
    if (!Object.keys(patch).length) return res.status(400).json({ ok: false, error: 'Nothing to update.' });
    const result = store.updateRow('pledges', previous.id, patch, { type: 'pledge', ...activityFor('pledges', { ...previous, ...patch }, previous, patch) });
    res.json({ ok: true, item: result });
  });
  ...
  app.post('/api/admin/passes', requireAdmin, (req, res) => {
    ...
    if (status !== 'active' || notes) store.updateRow('passes', info.lastInsertRowid, { status, notes }, { type: 'pass', action: status === 'revoked' ? 'status' : 'updated', summary: status === 'revoked' ? `${name}'s visitor pass revoked` : `Note added on ${name}'s pass` });
    res.status(201).json({ ok: true, item: rowFor('passes', info.lastInsertRowid) });
  });

  app.patch('/api/admin/passes/:id', requireAdmin, (req, res) => {
    ...
    if (!Object.keys(patch).length) return res.status(400).json({ ok: false, error: 'Nothing to update.' });
    const result = store.updateRow('passes', previous.id, patch, { type: 'pass', ...activityFor('passes', { ...previous, ...patch }, previous, patch) });
    res.json({ ok: true, item: result });
  });
  ...
  app.post('/api/admin/reservations', requireAdmin, (req, res) => {
    ...
    try {
      const info = stmts.insertReservation.run({ name, wc, trek, status, notes });
      res.status(201).json({ ok: true, item: rowFor('reservations', info.lastInsertRowid) });
    } catch (error) {
      if (String(error.message).includes('UNIQUE')) return res.status(409).json({ ok: false, error: 'This WeChat ID already has a seat or waitlist spot on this trek.' });
      throw error;
    }
  });
  ...
  app.patch('/api/admin/reservations/:id', requireAdmin, (req, res) => {
    ...
    const result = store.updateRow('reservations', previous.id, patch, { type: 'reservation', ...activityFor('reservations', next, previous, patch) });
    res.json({ ok: true, item: result });
  });
  ...
  for (const collection of COLLECTIONS) {
    app.delete(`/api/admin/${collection}/:id`, requireAdmin, (req, res) => {
      ...
      store.removeRow(collection, row.id, { type: TYPE_OF[collection], action: 'deleted', summary });
      res.json({ ok: true });
    });
  }
}
