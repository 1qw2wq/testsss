...
(() => {
  ...
  function showApp() {
    ...
    state.pollTimer = setInterval(() => {
      updateBadges().catch(() => {});
      const focusedInsideDesk = [els.view, els.modal, els.confirm, els.drawer].some((root) => root.contains(document.activeElement));
      if (!document.hidden && !state.detail && !focusedInsideDesk) renderRoute({ quiet: true });
    }, 45_000);
  }
    ...
    try {
      ...
      const eventCards = (data.events || []).map((event) => {
        ...
        return `<article class="admin-event-card">
          ...
          <p class="admin-event-location"><b>Appears in</b> ${esc(event.trek ? (TREK_LABEL[event.trek] || event.trek) : 'Calendar only')}</p>
      ...
      els.view.innerHTML = `<div class="page-head"><div><p class="eyebrow">Club desk · Community</p><h2 class="page-title">Events</h2><p class="kpi-sub">${nfmt(data.total)} event${data.total === 1 ? '' : 's'} managed here. Upcoming events appear on the homepage and public calendar; linked events also appear in their trek window.</p></div><div class="page-actions"><a class="btn btn-sm" href="/events">View public events ↗</a><button class="btn btn-ink btn-sm" type="button" data-act="add-event">＋ Add event</button></div></div>
        ${eventCards ? `<div class="admin-event-grid">${eventCards}</div>` : `<div class="empty card"><h3>No events yet.</h3><p>Add an event and choose its trek destination to show it in a trek window too.</p><button class="btn btn-ink btn-sm" type="button" data-act="add-event">＋ Add your first event</button></div>`}`;
      setLive('live');
      setAlert('');
    } catch (err) {
      if (location.hash !== startedHash) return;
  ...
  function bookGoalModal(goal) {
    els.modalCard.innerHTML = `<p class="eyebrow">Club desk · book drive</p><h2>Adjust the book goal</h2><p class="dialog-lead">This target is saved with the club settings and updates the progress displays on the desk and public site. The historical baseline is separate from saved pledges; Clear all data resets it to zero.</p>
  ...
  async function clearAllData() {
    const ok = await askConfirm(
      'Clear all club data?',
      'This permanently deletes every saved application, pledge, event, visitor pass, trek reservation, and activity-log entry. It also resets the historical book baseline to 0, so the public book total becomes 0. Your configured book goal is kept. The database stays cleared after restart. This cannot be undone.',
      'Clear all data',
      true,
      'CLEAR ALL DATA'
    );
    ...
    try {
      ...
      const baseline = result.cleared?.historicalBooks || 0;
      toast(`Cleared ${nfmt(records)} records, ${nfmt(history)} activity events, and reset ${nfmt(baseline)} historical books to 0.`);
    } catch (err) { toast(err.message); }
  }
